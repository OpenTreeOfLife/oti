...
public class QueryRunner extends OTIDatabase {
	...
	public final Index<Node> treeRootNodesByTreeOrStudyId = getNodeIndex(OTINodeIndex.TREE_ROOT_NODES_BY_TREE_ID_OR_STUDY_ID);
	...
	public Iterable<String> doBasicSearch(SearchableProperty search, String searchValue) {
        ...
        try {
			for (Node hit : hits) {
				sourceIds.add((String) hit.getProperty(OTINodeProperty.STUDY_ID.propertyName()));
			}
		} finally {
			hits.close();
		}
        ...
        if (search.equals(SearchableProperty.MAPPED_OTT_TAXON_NAME_TREE)) {
			hits = getNodeIndex(OTINodeIndex.TREE_ROOT_NODES_BY_MAPPED_TAXON_NAME_WHITESPACE_FILLED)
	...
	public List<Node> getRemoteSourceMetaNodesForSourceId(String sourceId) {
		...
		for (String remote : getKnownRemotes()) {
			Node sourceMeta = DatabaseUtils.getSingleNodeIndexHit(studyMetaNodesByOTProperty, remote + OTIConstants.SOURCE_ID_SUFFIX, sourceId);
			if (sourceMeta != null) {
	...
	public List<String> getKnownRemotes() {
		...
		String[] knownRemotesArr = (String[]) graphDb.getGraphProperty(OTIGraphProperty.KNOWN_REMOTES);
	...
	public List<String> getSourceIds(String location, Set<String> excludedSourceIds) {
		...
		IndexHits<Node> sourcesFound = studyMetaNodesByOTProperty.query(location + OTIConstants.SOURCE_ID_SUFFIX + ":*");
		try {
			while (sourcesFound.hasNext()) {
				String sid = (String) sourcesFound.next().getProperty(OTINodeProperty.STUDY_ID.propertyName());
	...
	public List<String> getTreeIdsForSourceId(String location, String sourceId, Set<String> excludedTreeIds) {
		...
		IndexHits<Node> hits = treeRootNodesBySourceId.query(location + OTIConstants.SOURCE_ID_SUFFIX + ":" + sourceId);
		try {
			while (hits.hasNext()) {
	 ...
	 *
	public HashSet<Node> getOTIsFromMetadataNode(Node sourceMeta){
	...
	public Node getSourceMetaNode(String sourceId, String location) {
		return DatabaseUtils.getSingleNodeIndexHit(studyMetaNodesByOTProperty, location + OTIConstants.SOURCE_ID_SUFFIX, sourceId);
	}
	...
	public Node getTreeRootNode(String treeId, String location) {
		return DatabaseUtils.getSingleNodeIndexHit(treeRootNodesByTreeOrStudyId, location + OTIConstants.TREE_ID_SUFFIX, treeId);
	}
	 ...
	 *
	@Deprecated
	public Map<String, Object> getMetadataForOTI(Node otu) {
	...
	public Map<String, Object> getMetadataForSource(Node sourceMeta) {
		...
		if (sourceMeta.getProperty(OTINodeProperty.LOCATION.propertyName()).equals(LOCAL_LOCATION)) {
			hasLocalCopy = true;
		} else {
			hasLocalCopy = false;
		}
		...
		// check for remotes
		for (Node remoteMeta : getRemoteSourceMetaNodesForSourceId((String) sourceMeta.getProperty(OTINodeProperty.STUDY_ID.propertyName()))) {
			remotes.add((String) remoteMeta.getProperty(OTINodeProperty.LOCATION.propertyName()));
		}

		// put it together and what have you got
		Map<String, Object> result = new HashMap<String, Object>();
		result.put("sourceId", sourceMeta.getProperty(OTINodeProperty.STUDY_ID.propertyName()));
		result.put("metadata", metadata);
	...
	public static Map<String, Object> getAlternativeMappingsForNode(Node otuNode) {
		...
		Iterable<Relationship> tnrsRels = otuNode.getRelationships(OTIRelType.TNRSMATCHFOR, Direction.INCOMING);
